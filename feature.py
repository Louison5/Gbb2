...
class FeatureAgent(MahjongGBAgent):
    ...
    observation: 6*4*9
        (145)*4*9
        # hand4+每个人(吃4+碰1+杠1)*4+每个人弃牌历史28*4+剩余牌
    action_mask: 235
    ...
    def _other_embedding_update(self):
        OBS_SIZE = 144
        ACT_SIZE = 235
        ...
        """embed the rest 140dims
        """
        # chi4peng1gang1 for everyone
        self.obs[4:]=0
        for p in range(4):
            for act , tile, offer in self.packs[p]:
                if tile!="CONCEALED":
                    if act=="CHI":
                        offset_layer=0
                        while self.obs[p*6+4+offset_layer,OFFSET_TILE[tile]+int(tile[1])-1]==1:
                            offset_layer+=1
                        self.obs[p*6+4+offset_layer,OFFSET_TILE[tile]+int(tile[1])-1]+=1
                    elif act=="PENG":
                        self.obs[p*6+4+4,OFFSET_TILE[tile]+int(tile[1])-1]+=1
                    elif act=="GANG":
                        self.obs[p*6+4+5,OFFSET_TILE[tile]+int(tile[1])-1]+=1
        #跳过了“自己暗杠”捏 可以补充！
        #now offset=4+6*4=28
        #history 28 everyone
            for round, tile in enumerate(self.history[p]):
                self.obs[28+28*p+round,+OFFSET_TILE[tile]+int(tile[1])-1]+=1
            pass
        # leftover 4 剩下的牌，还没完全想好，先全部放空
        # full = np.ones(36)*4
        #     # rid 
        # full[34:36]=0
        # for layer in range(OBS_SIZE-1):
        #     if layer in [8,14,20,26]:
        #         # Peng layers
        #         full=full-self.obs[layer]*3
        #     elif layer in [9,15,21,27]:
        #         # Gang layers
        #         full=full-self.obs[layer]*4
        #     elif layer in [4,5,6,7,10,11,12,13,16,17,18,19,22,23,24,25]:
        #         # Chi layers
        #         del_layer=np.zeros(36)
        #         for card_index ,existence in enumerate(self.obs[layer]):
        #             if existence:
        #                 del_layer[card_index]+=1
        #                 del_layer[card_index-1]+=1
        #                 del_layer[card_index+1]+=1
        #         full=full-del_layer
        #     else:
        #         full=full-self.obs[layer]
        # # below is to prevent Peng and Gang recalculation(重复计算)
        # full=np.where(full<0,0,full)
        # # self.obs[OBS_SIZE-1]
        # d = defaultdict(int)
        # for index,existence in enumerate(full):
        #     d[index] += 1
        # for index in d:
        #     self.obs[OBS_SIZE-4: OBS_SIZE-4 + d[index],index] = 1
